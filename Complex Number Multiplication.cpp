...
public:
    string complexNumberMultiply(string num1, string num2) {
        pair<int, int> a = getParam(num1), b = getParam(num2);
    ...
    pair<int, int> getParam(string num){
        int p = num.find('+');
        double real = stoi(num.substr(0, p));
        double imaginary = stoi(num.substr(p + 1, num.size() - p - 2));
